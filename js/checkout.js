    ...
    if (window.location.pathname.includes('checkout.html')) {
        ...
        if (finalizeBtn) {
            ...
            finalizeBtn.addEventListener('click', (e) => {
                ...
                if (!name || !phone) { showError("PLEASE PROVIDE YOUR NAME AND WHATSAPP NUMBER."); return; }
                if (deliveryMethod === 'delivery' && !address) { showError("WE NEED TO KNOW WHERE TO GO! PLEASE PROVIDE A DELIVERY ADDRESS."); return; }

                let message = `*NEW ORDER - DIPPED SHX*%0A%0A*Customer:* ${name}%0A*Phone:* ${phone}%0A*Method:* ${deliveryMethod.toUpperCase()}%0A`;
                if (deliveryMethod === 'delivery') message += `*Address:* ${address}%0A`;
        ...
        document.body.addEventListener('click', (e) => {
            if (e.target.closest('.add-to-cart-btn') || e.target.closest('.cart-plus') || e.target.closest('.cart-minus')) {
                setTimeout(window.renderCheckoutItems, 50);
            }
        });
